...
color_changed_cb (GSettings *settings, const gchar *key, gpointer data)
{
    ...
    else if (g_str_equal (key, "pressure-color")) {
        gdk_rgba_parse (&procdata->config.pressure_color, color);
        procdata->mem_graph->colors.at(2) = procdata->config.pressure_color;
    }
    else if (g_str_equal (key, "gonzocache-color")) {
        /* Drives both the second wiper's color (on the memory pie's
         * GsmColorButton, via set_color2 -- it has no slot in
         * LoadGraph::colors since a color-button wiper isn't a line
         * series) and colors.at(3), the GonzoCache line series in the
         * graph itself. Targets load_graph_get_mem_color_picker, not
         * the pressure picker -- the second wiper lives on the memory
         * pie now, per direct correction. */
        gdk_rgba_parse (&procdata->config.gonzocache_color, color);
        gsm_color_button_set_color2(
            GSM_COLOR_BUTTON(load_graph_get_mem_color_picker(procdata->mem_graph)),
            &procdata->config.gonzocache_color);
        procdata->mem_graph->colors.at(3) = procdata->config.gonzocache_color;
    }
...
procman_data_new (GSettings *settings)
{
    ...
    /* Gonzo: pressure_color is the pressure pie's single wiper color,
     * matching what the pie used before it became configurable
     * (orange, the original fixed color). gonzocache_color is the
     * memory pie's second wiper (dark blue, per the actual request),
     * moved here from the pressure pie after direct correction. Both
     * default strings were verified by actually round-tripping them
     * through gdk_rgba_parse()/gdk_rgba_to_string() rather than
     * hand-guessed -- two earlier hex-format guesses both silently
     * failed to parse. */
    color = g_settings_get_string (settings, "pressure-color");
    if (!color)
        color = g_strdup ("rgb(230,126,34)");
    g_signal_connect (G_OBJECT(settings), "changed::pressure-color",
                      G_CALLBACK(color_changed_cb), pd);
    gdk_rgba_parse(&pd->config.pressure_color, color);
    g_free (color);

    color = g_settings_get_string (settings, "gonzocache-color");
    if (!color)
        color = g_strdup ("rgb(28,38,115)");
    g_signal_connect (G_OBJECT(settings), "changed::gonzocache-color",
                      G_CALLBACK(color_changed_cb), pd);
    gdk_rgba_parse(&pd->config.gonzocache_color, color);
    g_free (color);
...
{
    Glib::set_application_name(_("Gonzo System Monitor"));
}
