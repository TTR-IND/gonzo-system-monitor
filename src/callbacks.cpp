...
cb_about (GtkAction *action, gpointer data)
{
    ...
    gtk_show_about_dialog (
        ...
        "program-name",       _("Gonzo System Monitor"),
        "version",            VERSION,
        "title",              _("About Gonzo System Monitor"),
...
/* Gonzo: color2-aware sibling of change_settings_color(). The primary
 * function hard-codes gsm_color_button_get_color(), which only ever
 * reads the button's primary color -- calling it for the pressure
 * pie's second wiper would silently persist color1's value a second
 * time instead of color2's actual value. This reads
 * gsm_color_button_get_color2() instead, otherwise identical. */
static void change_settings_color2(GSettings *settings, const char *key,
                   GSMColorButton *cp)
{
    const GdkRGBA *c = gsm_color_button_get_color2(cp);
    char *color = gdk_rgba_to_string (c);
    g_settings_set_string (settings, key, color);
    g_free (color);
}
...
void
cb_pressure_color_changed (GSMColorButton *cp, gpointer data)
{
    ProcData * const procdata = static_cast<ProcData*>(data);
    change_settings_color(procdata->settings, "pressure-color", cp);
}

void
cb_gonzocache_color_changed (GSMColorButton *cp, gpointer data)
{
    ProcData * const procdata = static_cast<ProcData*>(data);
    change_settings_color2(procdata->settings, "gonzocache-color", cp);
}
