...
proctable_new (ProcData * const procdata)
{
	...
	for multi-seat environments. See http://en.wikipedia.org/wiki/Multiseat_configuration */
        ...
        N_("Priority"),
        NULL,
        /* Gonzo: coldness percentage reported by detritus, sourced
         * directly from the kernel's own Referenced/Rss page-table
         * accounting (see detritus.c's select_cold_victims()), not a
         * userspace-tracked scan-cycle history. titles[] is only ever
         * indexed up to COL_PRIORITY by the column-construction loop
         * below (COL_SURFACE/COL_POINTER/COL_TOOLTIP are handled
         * separately and never read this array) -- this entry exists
         * purely so titles[] stays positionally aligned with the enum
         * for anyone reading it top-to-bottom, matching upstream's
         * existing convention of keeping a placeholder ("POINTER") for
         * COL_POINTER even though it too is never read from this
         * array. */
        N_("Coldness %"),
        "POINTER"
    };
    ...
    model = gtk_tree_store_new (NUM_COLUMNS,
                                ...
                                G_TYPE_INT,                 /* Gonzo: detritus idle streak */
    ...
    for (i = COL_USER; i <= COL_DETRITUS_COLDNESS; i++) {
        ...
        /* COL_SURFACE (the process icon) is already fully handled above,
         * merged into the expander column via a pixbuf cell renderer.
         * It sits between COL_PRIORITY and COL_DETRITUS_COLDNESS in the enum,
         * so extending this loop's upper bound to reach COL_DETRITUS_COLDNESS
         * means it now passes through COL_SURFACE for the first time --
         * skip it explicitly rather than let it fall through into the
         * generic text-column path below, which would build a second,
         * meaningless column for image data. */
        if (i == COL_SURFACE) {
            continue;
        }
        ...
        switch (i) {
            ...
            case COL_DETRITUS_COLDNESS:
                gtk_tree_view_column_set_cell_data_func(col, cell,
                                                        &procman::coldness_cell_data_func,
                                                        GUINT_TO_POINTER(i),
                                                        NULL);
                break;
...
update_info_mutable_cols(ProcInfo *info)
{
    ...
    tree_store_update(model, &info->node, COL_DETRITUS_COLDNESS, info->detritus_coldness_pct);
}
...
update_info (ProcData *procdata, ProcInfo *info)
{
    ...
    /* Gonzo: pure lookup against the snapshot refresh_list() already
     * refreshed once this cycle -- no file I/O per process. */
    info->detritus_coldness_pct = gonzo_detritus_get_coldness_pct(info->pid);
}
...
refresh_list (ProcData *procdata, const pid_t* pid_list, const guint n)
{
    ...
    /* Gonzo: one shared read of detritus's status snapshot per refresh
     * cycle, not one per process -- update_info() below consults this
     * already-parsed snapshot rather than re-reading the file itself. */
