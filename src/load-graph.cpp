...
get_memory (LoadGraph *graph)
{
    ...
    /* Gonzo: refresh detritus's status snapshot and reflect it via the
     * pressure pie (same gsm_color_button_set_fraction mechanism
     * mem/swap use, for visual and behavioral consistency) plus a
     * supplementary status label for state a pie can't represent --
     * "detritus not running" or which process is currently frozen.
     * gonzo_detritus_refresh() is cheap (one small file read, no
     * parsing library) and safe to call once per memory-graph tick
     * independently of proctable.cpp's own once-per-cycle call to the
     * same function -- both are just readers of the same snapshot;
     * this module's contract explicitly supports being polled from
     * more than one place.
     *
     * The pie's fill is driven by mem_rate_kb_per_sec -- the rate of
     * |MemAvailable| change between detritus's two most recent
     * publishes -- not by trickle_bytes_interval and not by PSI.
     * trickle_bytes_interval only reports detritus's own small
     * proactive contribution and says nothing about what the rest of
     * the system (a benchmark, the kernel's own reclaim, a browser) is
     * doing to memory; PSI measures kernel stall time, which can sit
     * at 0 on a healthy system regardless of how much memory is
     * moving. This dial is specifically "how fast is RAM filling up or
     * emptying right now", which is what a rate of MemAvailable change
     * actually measures. */
    gonzo_detritus_refresh();
    {
        gdouble rate_fraction = 0.0;  /* pie shows 0 (empty) when no data */

        /* Ceiling: percent of total RAM changing per second, not a
         * fixed byte count -- a fixed number would be meaningless
         * across machines with different amounts of RAM (10MB/s means
         * something very different on an 8GB box than a 64GB one).
         * 5%/sec swing of total memory = full pie. mem.total here is
         * in bytes (from glibtop_get_mem() above); convert to KB to
         * match mem_rate_kb_per_sec's units. */
        const gdouble RATE_CEILING_PCT_PER_SEC = 0.05;
        gdouble mem_total_kb = (gdouble)mem.total / 1024.0;
        gdouble rate_ceiling_kb_per_sec =
            mem_total_kb * RATE_CEILING_PCT_PER_SEC;

        /* Gonzo: combined "ZRAM value (pressure rate)" line, one label,
         * not two stacked widgets. labels.swap already holds ZRAM's
         * plain-text value ("X bytes (Y%) of Z") from
         * set_memory_label_and_picker() above -- retrieve it, escape
         * it (gtk_label_set_markup interprets the whole string as
         * Pango markup, so anything not already markup-safe needs
         * escaping even though a formatted byte string is unlikely to
         * contain special characters in practice), and append the
         * pressure clause in parentheses to form one real sentence
         * rather than two disconnected lines. */
        const gchar *zram_text = gtk_label_get_text(GTK_LABEL(graph->labels.swap));
        gchar *zram_escaped = g_markup_escape_text(zram_text, -1);

        gchar *pressure_clause;
        if (!gonzo_detritus_is_available()) {
            pressure_clause = g_strdup_printf("<i>%s</i>", _("detritus not running"));
        } else if (gonzo_detritus_is_frozen()) {
            pressure_clause = g_strdup_printf(
                "<span foreground=\"#c0392b\"><b>%s</b> %s</span>",
                _("Paused:"), gonzo_detritus_get_frozen_name());
        } else {
            gdouble rate = gonzo_detritus_get_mem_rate_kb_per_sec();
            if (rate < 0) {
                pressure_clause = g_strdup_printf("<i>%s</i>", _("no rate data"));
            } else {
                rate_fraction = (rate_ceiling_kb_per_sec > 0)
                    ? CLAMP(rate / rate_ceiling_kb_per_sec, 0.0, 1.0)
                    : 0.0;

                char *rate_str = g_format_size_full(
                    (guint64)(rate * 1024), G_FORMAT_SIZE_IEC_UNITS);
                if (rate_fraction >= 0.75) {
                    pressure_clause = g_strdup_printf(
                        "<span foreground=\"#c0392b\">%s/s</span>", rate_str);
                } else if (rate_fraction >= 0.35) {
                    pressure_clause = g_strdup_printf(
                        "<span foreground=\"#d68910\">%s/s</span>", rate_str);
                } else {
                    pressure_clause = g_strdup_printf("%s/s", rate_str);
                }
                g_free(rate_str);
            }
        }

        gchar *combined = g_strdup_printf("%s (%s)", zram_escaped, pressure_clause);
        gtk_label_set_markup(GTK_LABEL(graph->labels.swap), combined);

        g_free(zram_escaped);
        g_free(pressure_clause);
        g_free(combined);

        if (graph->swap_color_picker)
            gsm_color_button_set_fraction2(GSM_COLOR_BUTTON(graph->swap_color_picker),
                                           rate_fraction);

        graph->data[0][2] = (gfloat)rate_fraction;
    }

    /* Gonzo: GonzoCache usage second wiper + fourth line series. Real
     * page-cache residency (mincore()-verified, computed by detritus.c
     * across whatever GonzoCache actually preloaded this session --
     * see gonzo-detritus.h's gonzo_detritus_get_gonzocache_resident_kb()
     * for the full chain), expressed as a fraction of total RAM to
     * match every other series in this graph. This is a real,
     * measured figure, not an estimate -- detritus mmaps each
     * preloaded file and asks the kernel directly which of its pages
     * are currently resident. */
    {
        gulong resident_kb = gonzo_detritus_get_gonzocache_resident_kb();
        gdouble mem_total_kb = (gdouble)mem.total / 1024.0;
        gdouble gonzocache_fraction = (mem_total_kb > 0)
            ? CLAMP((gdouble)resident_kb / mem_total_kb, 0.0, 1.0)
            : 0.0;

        if (graph->mem_color_picker)
            gsm_color_button_set_fraction2(GSM_COLOR_BUTTON(graph->mem_color_picker),
                                           gonzocache_fraction);

        graph->data[0][3] = (gfloat)gonzocache_fraction;
    }
}
    ...
    swap_color_picker(NULL)
{
    ...
    case LOAD_GRAPH_MEM:
        n = 4;  /* Gonzo: mem, swap/zram, pressure, GonzoCache -- was 2, then 3 */
    ...
    case LOAD_GRAPH_MEM:
        ...
        colors[2] = ProcData::get_instance()->config.pressure_color;
        colors[3] = ProcData::get_instance()->config.gonzocache_color;
