...
struct ProcConfig
{
     ...
     * (defaults to orange, matching the pie's original fixed color
     * before it became configurable). gonzocache_color is the
     * memory pie's second wiper, showing GonzoCache page-cache usage
     * layered on top of primary memory usage (defaults to dark blue,
     * per the actual request) -- moved here from the pressure pie
     * after direct correction; the color was always GonzoCache's own,
     * never conceptually tied to pressure. */
    GdkRGBA     pressure_color;
    GdkRGBA     gonzocache_color;
...
MutableProcInfo()
  : disk_write_bytes_current(0ULL),
    ...
    /* Gonzo: how cold this process's memory is (0-100%) per the
     * kernel's own page-table Referenced/Rss accounting, as computed
     * by detritus's select_cold_victims() -- not a userspace-tracked
     * scan-cycle history. -1 means no data -- either the detritus
     * status file is absent/unreadable, or this pid was not in
     * detritus's own candidate list on the last read (e.g. below its
     * RSS floor, or on the skip list). Never treat -1 as "definitely
     * 0% cold" -- it means "unknown", not "hot". */
    gint detritus_coldness_pct;
};
