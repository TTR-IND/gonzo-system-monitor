...
namespace procman
{
    ...
    /*
     * Gonzo: renders the detritus coldness column (COL_DETRITUS_COLDNESS).
     * -1 is the documented sentinel for "no detritus data" (daemon not
     * running, or this pid absent from detritus's candidate list) --
     * rendered as italic "N/A", matching memory_size_na_cell_data_func's
     * existing convention just above. Real coldness values (0-100)
     * render as a percentage.
     */
    void coldness_cell_data_func(GtkTreeViewColumn *col, GtkCellRenderer *renderer,
                                    GtkTreeModel *model, GtkTreeIter *iter,
                                    gpointer user_data)
    {
        const guint index = GPOINTER_TO_UINT(user_data);
        ...
        gint coldness = g_value_get_int(&value);
        g_value_unset(&value);
        ...
        if (coldness < 0) {
            ...
            char *str = g_strdup_printf ("%d%%", coldness);
            g_object_set(renderer, "text", str, NULL);
            g_free(str);
        }
    }
